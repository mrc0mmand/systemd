...
for phase in "${PHASES[@]}"; do
    case $phase in
        ...
        RUN_ASAN)
            ...
            $DOCKER_EXEC sh -c "UBSAN_OPTIONS=print_stacktrace=1:print_summary=1:halt_on_error=1 meson test --wrapper /build/travis-ci/tools/wrapper -v --timeout-multiplier=3 -C ./build/ --print-errorlogs"
            ;;
        CLEANUP)
            info "Cleanup phase"
